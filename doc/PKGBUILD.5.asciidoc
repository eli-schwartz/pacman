...
These are directly sourced and executed by makepkg, so anything that Bash or
the system has available is available for use here. Be sure any exotic commands
...
*prepare() Function*::
	...
	is skipped. For example, generating configuration files using autoreconf
	results in universally applicable changes and should occur in `prepare()`,
	but running the configure script results in changes specific to the current
	build, and must occur exclusively in `build()`. Features like `buildflags`
	or `makeflags` are expressly not available during `prepare()`.
...
*build() Function*::
	...
	root, should be performed here. This function should be idempotent, in case
	`makepkg --noextract` is used multiple times.
